...
void usb_cdc_zbs_prog_mode_process_tx(void) {
    uint8_t ep_num = usb_cdc_get_port_data_ep(USB_CDC_ZBS_PROG_PORT);
    usb_cdc_state_t *cdc_state = &usb_cdc_states[USB_CDC_ZBS_PROG_PORT];
    ...
    while((count = circ_buf_count_to_end(tx_buf->head, tx_buf->tail, USB_CDC_BUF_SIZE))) {
        zbs_prog_process_input(&tx_buf->data[tx_buf->tail], count);
...
static void usb_cdc_port_tx_complete(int port) {
    ...
    if (port != USB_CDC_ZBS_PROG_PORT) {
        usb_cdc_port_start_tx(port);
    } else {
        usb_cdc_zbs_prog_mode_process_tx();
    }
...
void usb_cdc_reconfigure_port_pin(int port, cdc_pin_t pin) {
    if (port < (USB_CDC_NUM_PORTS-1) && pin < cdc_pin_last) {
...
void usb_cdc_reconfigure() {
    for (int port = 0; port < USB_CDC_NUM_PORTS-1; port++) {
